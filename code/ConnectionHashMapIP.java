...
public class ConnectionHashMapIP{
  ...
  public void printHMConnectionData() {
    ...
    boolean color = true;
    System.out.println();
    System.out.println("      Client     ->    Server       OPEN  CLOSED");
    mHashMap.forEach((key, connIPObj) -> {
      if(connIPObj.getOpen() > 1) {
        System.out.printf(addColorCode + "%-16s -> %-16s %-6s %-6s\n", connIPObj.getSource(), connIPObj.getDest(), connIPObj.getOpen(), connIPObj.getClose());
      } else {
        System.out.printf(restoreColorCode + "%-16s -> %-16s %-6s %-6s\n", connIPObj.getSource(), connIPObj.getDest(), connIPObj.getOpen(), connIPObj.getClose());
      }
    });
